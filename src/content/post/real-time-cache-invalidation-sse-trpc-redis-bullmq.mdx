...
- UI updates seamlessly, with no loading spinner and no page refresh
...
- **Type-safe registry**: define a Zod schema per job name. `runJob("email.send", data)` is fully typed, so wrong payload shapes are caught at compile time.
